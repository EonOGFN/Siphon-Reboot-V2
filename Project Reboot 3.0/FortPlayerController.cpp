	...
	if (Fortnite_Version > 1.8 || Fortnite_Version == 1.11)
	{
		...
		*(AActor**)(__int64(DeathInfo) + MemberOffsets::DeathInfo::FinisherOrDowner) = KillerPlayerState ? KillerPlayerState : DeadPlayerState;
		...
		if (AmountOfHealthSiphon != 0)
		{
			...
			if (KillerPawn && KillerPawn != DeadPawn) {
				auto PC = Cast<AFortPlayerControllerAthena>(KillerPawn->GetController());
				if (Globals::Siphon) {
					if (KillerPawn) {
						auto WorldInventory = PC->GetWorldInventory();

						static auto WoodItemData = FindObject<UFortItemDefinition>(L"/Game/Items/ResourcePickups/WoodItemData.WoodItemData");
						static auto StoneItemData = FindObject<UFortItemDefinition>(L"/Game/Items/ResourcePickups/StoneItemData.StoneItemData");
						static auto MetalItemData = FindObject<UFortItemDefinition>(L"/Game/Items/ResourcePickups/MetalItemData.MetalItemData");

						auto KillerPC = KillerPawn->GetController();
						if (KillerPC) {
							auto KillerStateSigma = Cast<AFortPlayerStateAthena>(KillerPC->GetPlayerState());
							if (KillerStateSigma) {
								auto AbilitySystemComponent = KillerStateSigma->GetAbilitySystemComponent();
								if (AbilitySystemComponent) {
									...
									Tag.TagName = Cue;
									AbilitySystemComponent->NetMulticast_InvokeGameplayCueAdded(Tag, FPredictionKey(), Handle);
									AbilitySystemComponent->NetMulticast_InvokeGameplayCueExecuted(Tag, FPredictionKey(), Handle);
								}
							}
						}

						if (PC) {
							WorldInventory->AddItem(WoodItemData, nullptr, Globals::SiphonMaterials);
							WorldInventory->AddItem(StoneItemData, nullptr, Globals::SiphonMaterials);
							WorldInventory->AddItem(MetalItemData, nullptr, Globals::SiphonMaterials);
						}
					}
				}
			}
